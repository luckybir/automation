...
function jdAuto() {
  ...
  while (!taskFinish) {
    ...
    for (let i = 0; i < taskList.length; i++) {
      ...
      // sometime 去完成 is only one stack, if two stack then taskList[i].parent().parent()
      ...
      switch (taskCategory) {
        ...
        case "cartGoods":
          ...
          for (let j = 0; j < coins.length; j++) {
            ...
            if (coins[j].parent().child(0).text() != "已加购") {
              // log(j);
              delay(2);
...
function getTaskCategory(taskTitle, taskSubTitle) {
  ...
  regTaskSubTitle = /^成功浏览加购\d+个商品可得/g;
  if (regTaskSubTitle.test(taskSubTitle)) {
